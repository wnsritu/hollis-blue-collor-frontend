    ...
    try {
      ...
      toast.success(
        <div>
          ...
          <div className="text-xs opacity-90">Our safety team will review this conversation within 24 hours.</div>
        </div>
      );
      setReportOpen(false);
      setReportDetails("");
                    ...
                    <div key={m.id || idx} className={cn("flex", mine ? "justify-end" : "justify-start")}>
                      ...
                      >
                        ...
                        <div className={cn("flex items-center justify-end gap-1 text-[10px]", mine ? "opacity-80" : "text-muted-foreground")}>
                          ...
                          {mine && (
                            <span title={m.is_read || m.read ? "Read" : "Sent / Delivered"}>
                              {m.is_read || m.read ? (
                                <CheckCheck size={14} className="text-emerald-400 font-bold" />
                              ) : (
