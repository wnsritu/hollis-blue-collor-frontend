 ...
 *
 * Supports multiple values because the backend may return:
 * - boolean true
 * - numeric 1
 * - "verified"
 * - "approved"
 * - onboarding statuses that indicate approval
 */
function checkIsVerified(
  ...
  const dbVerified = String(verifiedRaw || "")
    .toLowerCase()
    .trim();
...
/**
 * Restricted state shown to providers who are not yet approved.
 */
...
export const providerNav: NavItem[] = [
  ...
  { to: "#", label: "Subscription", icon: CreditCard, exact: true },
  { to: "#", label: "Earnings", icon: Wallet, matchPaths: ["/provider/earnings"] },
  { to: "#", label: "Featured Listings", icon: Sparkles, matchPaths: ["/provider/featured", "/provider/sponsored"] },
  {
    to: "/provider/profile",
...
/**
 * Pick navigation items by label.
 */
const pick = (
  nav: NavItem[],
  labels: string[]
...
/**
 * Generate initials for account avatar.
 */
...
export function ProviderPortal({ children }: { children?: ReactNode }) {
  ...
  const fetchProfile = async (
    ...
    try {
      const res =
        await providerApi.getMyMarketplaceProfile();
...
/**
 * Customer portal shell — sidebar + top bar.
 */
